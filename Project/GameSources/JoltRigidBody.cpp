...
namespace basecross
{
    JoltRigidBody::JoltRigidBody(const shared_ptr<GameObject>& owner)
        : Rigidbody(owner),
          m_bodyID(JPH::BodyID::cInvalidBodyID)
    {
    }

    void JoltRigidBody::Initialize(const Settings& settings)
    {
        m_settings = settings;
    ...
    void JoltRigidBody::OnCreate()
    {
    }

    void JoltRigidBody::OnUpdate()
    {
    ...
    void JoltRigidBody::AddForce(const DirectX::XMFLOAT3& force)
    {
        if (!m_pPhysicsSystem || m_bodyID.IsInvalid()) return;
        m_pPhysicsSystem->GetBodyInterface().AddForce(m_bodyID, JPH::Vec3(force.x, force.y, force.z));
    }

    void JoltRigidBody::SetLinearVelocity(const DirectX::XMFLOAT3& velocity)
    {
        if (!m_pPhysicsSystem || m_bodyID.IsInvalid()) return;
